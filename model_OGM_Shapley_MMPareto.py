...
from hyper_OGM_Shapley_MMPareto import HyperGCN
...
class Model(nn.Module):
    ...
    def __init__(self, base_model, D_m, D_g, D_p, D_e, D_h, D_a, graph_hidden_size, fusion_type, n_speakers, max_seq_len, window_past, window_future,
        ...
        if self.graph_type=='hyper':
            self.graph_model = HyperGCN(a_dim=D_g, v_dim=D_g, l_dim=D_g, n_dim=D_g, nlayers=64, nhidden=graph_hidden_size, nclass=n_classes,
                                        dropout=self.dropout, lamda=0.5, alpha=0.1, variant=True, return_feature=self.return_feature, use_residue=self.use_residue, fusion_type=fusion_type, n_speakers=n_speakers, modals=self.modals, use_speaker=self.use_speaker, use_modal=self.use_modal, num_L=num_L, num_K=num_K)
        ...
        if self.multi_modal:
            ...
            if self.att_type == 'concat_subsequently':
                ...
                if self.use_residue:
                    ...
                    self.smax_fc_avl = nn.Linear(28, 768)
                else:
                    self.smax_fc = nn.Linear((graph_hidden_size*2)*len(self.modals), n_classes)
                    # self.smax_fc = nn.Linear(1536, n_classes)
                    self.smax_fc_avl = nn.Linear(28, 2)
                    self.fc_out = nn.Linear(18, 21)
    ...
    def forward(self, U, qmask, umask, seq_lengths, dataset, U_a=None, U_v=None, epoch=None):
        total_sum = sum(seq_lengths)
        #=============roberta features
        [r1,r2,r3,r4]=U
        ...
        if self.graph_type=='GCN3' or self.graph_type=='DeepGCN':
            ...
            self.graph_model.mode = "feature"
            emotions_feat_all, a, v, l= self.graph_model(features_a, features_v, features_l, seq_lengths, total_sum, dataset, qmask, epoch, pad_audio=False, pad_visual=False, pad_language=False)

            emotions_feat = self.dropout_(emotions_feat_all)
            emotions_feat = nn.ReLU()(emotions_feat)
            log_prob = F.log_softmax(self.smax_fc(emotions_feat), 1)

            self.graph_model.mode = "classify"
            emotions_feat_vl = self.graph_model(features_a, features_v, features_l, seq_lengths, total_sum, dataset, qmask, epoch, pad_audio=True, pad_visual=False, pad_language=False)
            emotions_feat_al = self.graph_model(features_a, features_v, features_l, seq_lengths, total_sum, dataset, qmask, epoch, pad_audio=False, pad_visual=True, pad_language=False)
            emotions_feat_av = self.graph_model(features_a, features_v, features_l, seq_lengths, total_sum, dataset, qmask, epoch, pad_audio=False, pad_visual=False, pad_language=True)
            emotions_feat_l = self.graph_model(features_a, features_v, features_l, seq_lengths, total_sum, dataset, qmask, epoch, pad_audio=True, pad_visual=True, pad_language=False)
            emotions_feat_v = self.graph_model(features_a, features_v, features_l, seq_lengths, total_sum, dataset, qmask, epoch, pad_audio=True, pad_visual=False, pad_language=True)
            ...
            print("There are no such kind of graph")        
        return log_prob, log_prob_pad, a, v, l, emotions_feat_all, emotions_feat_vl, emotions_feat_al, emotions_feat_av,\
               emotions_feat_l, emotions_feat_v, emotions_feat_a, emotions_feat_zero, \
               edge_index, edge_norm, edge_type, edge_index_lengths
