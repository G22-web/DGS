...
def train_or_eval_graph_model(model, loss_function, dataloader, epoch, cuda, modals, scheduler=None, optimizer=None,
    ...
    for data in dataloader:
        ...
        if train:
            ...
            with open("text/MELD_train_loss_all.txt", 'w') as train_los:
                train_los.write(str(losses))

            with open("text/MELD_train_raiov_all.txt", 'w') as train_raiov:
                train_raiov.write(str(ratios_v))

            with open("text/MELD_train_raioa_all.txt", 'w') as train_raioa:
                train_raioa.write(str(ratios_a))

            with open("text/MELD_train_raiol_all.txt", 'w') as train_raiol:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--l2', type=float, default=0.00003, metavar='L2', help='0.00003, L2 regularization weight')
    ...
    parser.add_argument('--alpha', type=float, default=0.2, help='0.2, alpha')
    ...
    parser.add_argument('--Deep_GCN_nlayers', type=int, default=4, help='no use,Deep_GCN_nlayers')
    ...
    parser.add_argument('--fusion_method', default='concat', type=str, choices=['sum', '默认：concat', 'gated', 'film'])

    parser.add_argument('--modulation', default='OGM_GE', type=str, choices=['Normal', 'OGM', '默认：OGM_GE'])
    ...
    parser.add_argument('--lr_decay_ratio', default=0.8, type=float, help='0.1, decay coefficient')
